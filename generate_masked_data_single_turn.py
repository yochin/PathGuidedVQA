...
def apply_blur_to_masked_area(image, mask, blur_strength=15):
    ...
    Parameters:
    - image: 블러링을 적용할 원본 이미지 (numpy array).
    - mask: 블러링을 적용할 영역을 나타내는 마스크 이미지 (numpy array, binary).
    - blur_strength: 블러링 강도 (커널 크기, 홀수로 지정).
    
    Returns:
    - result_image: 블러링이 적용된 결과 이미지 (numpy array).
    """
    # 마스크를 binary 형태로 변환 (필요한 경우)
    if len(mask.shape) == 3:
    ...
    # 원본 이미지에서 마스킹된 부분을 제외한 영역
    non_blurred_part = cv2.bitwise_and(image, image, mask=inverted_mask)

    # 블러링된 이미지에서 마스킹된 부분
    blurred_part = cv2.bitwise_and(blurred_image, blurred_image, mask=binary_mask)

    # 블러링된 부분과 원본의 나머지 부분을 합침
...
def main():
    ...
    use_det_info = conf['yolo']['use_det_info']
    ...
    # for calculating avg_infer_time
    file_count_inf_time = 1
    avg_infer_time=0.0
    ...
    for img_file in image_files:
        ...
        _cv_h, _cv_w, _ = cv_org_img.shape

        assert _cv_h == whole_height
        assert _cv_w == whole_width

        depth_path = os.path.join(depth_base, img_file_wo_ext + '.npy')
        if os.path.exists(depth_path):
            logging.info(f'{depth_path} exists. Load it.')
            depth_image = np.load(depth_path)       # h x w
        else:
            logging.info(f'{depth_path} does not exist. Generate and Save it.')
        ...
        _cv_d_h, _cv_d_w = depth_image.shape

        assert _cv_d_h == whole_height
        assert _cv_d_w == whole_width
        ...
        for i_gp, goal_label_cxcy in enumerate(list_labels_gps):
            ...
            if use_org_image:
                ...
                # depth_image: h x w (no ch)
                # depth_mask: h x w x 3 from depth_mask
                ...
                if dst_masking_depth:
                    ...
                    depth_starting_point = depth_image[whole_height-1, int(whole_width/2)]
                    depth_goal_point = depth_image[int(goal_cxcy[1]*whole_height), int(goal_cxcy[0]*whole_width)]
                    half_buffer_dist = abs((depth_goal_point - depth_starting_point)/2)
                    depth_mask_front = create_depth_mask(depth_image, [int(goal_cxcy[0]*whole_width), int(goal_cxcy[1]*whole_height)], 
                                            radius=25, buffer_dist=-half_buffer_dist)
                ...
                # depth_mask_near_path: h x w x 3
                ...
                if dst_use_path_mask:
                    dict_masks['D'] = depth_mask_near_path.copy()   # deep copy

                    # if dst_masking_depth:
                    #     # dict_masks['D'] = cv2.bitwise_and(dict_masks['D'], depth_mask_front)
                    #     apply_blur_to_masked_area()
                ...
                # for debugging
                dict_masks_debug = {
                    'L1': dict_masks['L'],                  # h x w x 3
                    'R1': dict_masks['R'],                  # h x w x 3
                    'Depth_limit': depth_mask,              # h x w x 3 
                    'Path_half2m': depth_mask_near_path,    # h x w x 3
                ...
                if dst_use_path_mask and dst_masking_depth:
            ...
            # resize final displayed image to become h=800
            fixed_img_h = 800
            resize_ratio = float(fixed_img_h) / float(whole_height)
            fixed_img_w = int(resize_ratio * whole_width)
            img_note.resize((fixed_img_w, fixed_img_h))
            ...
            avg_infer_time = ((avg_infer_time * (file_count_inf_time - 1)) + inference_time) / file_count_inf_time
            logging.info(f"\t{file_count_inf_time}: AVG. Inference Time: {avg_infer_time} secs\n\n\n\n")
