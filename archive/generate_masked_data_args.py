...
def parse_args():
     ...
     parser.add_argument(
         '--images', metavar='DIRECTORY', required=True,
         help='directory which contains images and object properties')
     
     parser.add_argument(
         '--depths', metavar='DIRECTORY', required=True,
         help='directory which contains images and object properties')
     
     parser.add_argument(
         '--det-anno-pred', metavar='DIRECTORY', required=True,
         help='directory which contains images and object properties')
     
     parser.add_argument(
         '--det-anno-gt', metavar='DIRECTORY', required=True,
         help='directory which contains images and object properties')
     
     parser.add_argument(
         '--gp-info', metavar='DIRECTORY', required=True,
         help='directory which contains images and object properties')

     parser.add_argument(
         '--paths', metavar='DIRECTORY', required=True,
         help='directory which contains images and object properties')
...
def find_depth_difference_point(depth_image, depth_difference=5.0, crop_width=10, set_lower_middle_as_zero=True):
...
def get_intrinsic_ratio(filename):
    ...
    if 'MP_' in filename:   # MobilePhone
        fx_r = 4.3 / 6.17       # 0.6969
        fy_r = 4.3 / 4.55       # 0.9451
    
    elif 'ZED' in filename: # ZED
...
def main():
    ...
    depth_base = args.depths
    anno_path1 = args.det_anno_pred
    anno_path_gt = args.det_anno_gt
    anno_path_gp = args.gp_info    # annotation has GP in txt file
    path_to_path = args.paths
    ...
    # depth anything v2
    dep_any = depth_anything('vitl', 'vkitti', 80)

    yolo_dynamic = YOLO('yolo/WEIGHTS/aihub_pwalk/train852/weights/best.pt', task='detect')
    yolo_static = YOLO('yolo/WEIGHTS/gd_demo/train958/weights/best.pt', task='detect')
    ...
    for img_file in image_files:
        ...
        if os.path.exists(depth_path):
            ...
            print(f'{depth_path} is not exists. Generate and Save it.')
        ...
        if use_gp == 'det':
            ...
            if os.path.exists(path_to_gp):
                ...
                res_gp = find_depth_difference_point(depth_image, depth_difference=10.0, crop_width=10, set_lower_middle_as_zero=True)
                ...
                print(f'{path_to_gp} file is not exists. set a center as gp!')
        ...
        for i_gp, goal_label_cxcy in enumerate(list_labels_gps):
            ...
            if os.path.exists(json_path):
                ...
                # path_suc = False
                # path_array = None
                res_dict_path, depth_start_yx, depth_goal_yx = depth_lpp(depth_image, goal_cxcy)    # depth_sized_path is returned

                if 'successfully' in res_dict_path['result']:
                    path_suc = True
                    path_array_unnorm = res_dict_path['path']
            ...
            dict_masks = generate_mask(cv_org_img_pt, path_array_xy, line_thickness=200)    # ['L', 'R'], all left and right area along with path.
            ...
            output_dict = {
                ...
                'recommend_query': final_query[4],
                'recommend': final_description[4].replace('\n', ' '),
                'bboxes': bboxes,
            ...
            draw_path_on_scaled_image(depth_image, path_array_unnorm, depth_start_yx, depth_goal_yx, img, filename=path_to_debug, radius=20)
