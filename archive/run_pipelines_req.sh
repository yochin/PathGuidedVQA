...
task_name="ReQuest_DesPres4_LCR_HBO"
...
# CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6 python generate_data_by_yochin.py --llava-model-dir ../llm_models/llava/llava-v1.6-34b --model-name llava16_cli --db-dir ../val100 --prompt-id 41819 --output ../output_${task_name}_t11 | tee ../log_${task_name}_t11.txt
# CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6 python generate_data_by_yochin.py --llava-model-dir ../llm_models/llava/llava-v1.6-34b --model-name llava16_cli --db-dir ../val100 --prompt-id 41859 --output ../output_${task_name}_t16 | tee ../log_${task_name}_t16.txt
...
#     python prepare_eval.py --gt-dir ../output_${task_name}_t"$tt"/qa --output-dir ../output_${task_name}_t"$tt"/eval --prompt-id 41819 | tee ../log_${task_name}_t"$tt"_step2.txt
# done
# for tt in 16
# do
#     # just remove special tokens and merge into one long sentences.
...
#     python prepare_eval.py --gt-dir ../output_${task_name}_t"$tt"/qa --output-dir ../output_${task_name}_t"$tt"/eval --prompt-id 41859 | tee ../log_${task_name}_t"$tt"_step2.txt
# done
