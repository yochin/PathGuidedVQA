...
def evaluate_xmls(pred_xml_list, gt_dir, cache_dir, with_conciseness):
    # 결과를 저장할 디렉토리 생성
    ...
    avg_scores['word_count'] = {tag: 0.0 for tag in eval_gt_tags}
    ...
    for pred_xml_idx, pred_xml_path in enumerate(pred_xml_list):
        ...
        tag_scores[xml_filename]['word_count'] = {tag: [] for tag in eval_gt_tags}

        cached_data = None
        # 캐시 파일이 있으면 불러오기
        cache_path = os.path.join(cache_dir, f'{xml_filename}.pkl')
        if os.path.exists(cache_path):
            print(f"Loading cached result for {xml_filename}")
            with open(cache_path, 'rb') as f:
                cached_data = pickle.load(f)

            cached_output_desc_per_tag = cached_data['output_desc']
            cached_gt_desc_per_tag = cached_data['gt_desc']
            cached_llm_reason_scores_per_tag = cached_data['llm_reason_scores']
            cached_llm_scores_per_tag = cached_data['llm_scores']
        else:
            cached_data = None
        ...
        if cached_data is None:
            output_desc_per_tag = {}
            gt_desc_per_tag = {}
            llm_reason_scores_per_tag = {}
            llm_scores_per_tag = {}

        for i, gt_tag in enumerate(eval_gt_tags):
            print(f"Processing the file: {xml_filename}")
            ...
            if (cached_data is not None) and (cached_output_desc_per_tag[gt_tag] == output_desc) and (cached_gt_desc_per_tag[gt_tag] == gt_dest_desc):
                llm_reason_score = cached_llm_reason_scores_per_tag[gt_tag]
                llm_score = cached_llm_scores_per_tag[gt_tag]
                print(f"Loaded llm_reason_score from cache files")
            else:
                if with_conciseness:
            ...
            if cached_data is None:
                llm_reason_scores_per_tag[gt_tag] = llm_reason_score
                llm_scores_per_tag[gt_tag] = llm_score
                output_desc_per_tag[gt_tag] = output_desc
                gt_desc_per_tag[gt_tag] = gt_dest_desc

            print(f"  GT  : {gt_dest_desc}")
            print(f"  Pred: {output_desc}")
            print(f"  LLM Score: {llm_score} (Avg: {avg_scores['llm_scores'][gt_tag]})")

            num_words = len(output_desc.split())
            tag_scores[xml_filename]['word_count'][gt_tag].append(num_words)
            avg_scores['word_count'][gt_tag] += (num_words - avg_scores['word_count'][gt_tag]) / eval_counts[gt_tag]
            print(f"  Num words: {num_words} (Avg: {avg_scores['word_count'][gt_tag]})")

        # llm_score와 llm_reason_score만 저장
        if cached_data is None:
            cache_data = {
                'llm_reason_scores': llm_reason_scores_per_tag,
                'llm_scores': llm_scores_per_tag,
                'output_desc': output_desc_per_tag,
                'gt_desc': gt_desc_per_tag
            }
...
def evaluate_folders(pred_db_dir, gt_db_dir, output_dir, with_conciseness):
    # data setting
    pred_xml_list = glob.glob(f'{pred_db_dir}/**/*xml', recursive=True)
    pred_xml_list = sorted(pred_xml_list)

    cache_dir = os.path.join(output_dir, 'cache')
    tag_scores, avg_scores, error_file_list, eval_counts = evaluate_xmls(pred_xml_list, gt_db_dir, cache_dir=cache_dir, with_conciseness=with_conciseness)
...
def main():
    ...
    # # LLM Judge w/ conciseness
    # output_dir = os.path.join(conf['output_dir'], conf['task_name'], 'eval_llm_judge_conciseness')
    # evaluate_folders(pred_db_dir, eval_db_dir, output_dir, with_conciseness=True)

    # LLM Judge
    output_dir = os.path.join(conf['output_dir'], conf['task_name'], 'eval_llm_judge')
    evaluate_folders(pred_db_dir, eval_db_dir, output_dir, with_conciseness=False)
